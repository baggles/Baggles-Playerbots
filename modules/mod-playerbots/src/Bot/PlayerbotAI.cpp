...
uint32 PlayerbotAI::GetEquipGearScore(Player* player)
{
    ...
    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        // Use the same stat-weight system bots use for evaluating new items
        StatsWeightCalculator calc(player);
        float score = calc.CalculateItem(proto->ItemId, it->GetItemRandomPropertyId(), slot);

        sumScore += score;

        std::cout << "[GEAR DEBUG] Slot " << (int)slot << " Item: " << proto->Name1 << " Score: " << score << std::endl;
    }

    /* ---------- 2. Average the stat score ------------------------ */
    const uint8 divisor = ignoreOffhand ? TOTAL_SLOTS - 1 : TOTAL_SLOTS;
    return (uint32)(sumScore / divisor);
}
